...
| 2026-09-09 | Regra nova: **em iOS o ambiente é o dev build; `localhost` deixa de ser default; `Info.plist` só via `app.json`** | CARD-037. O Simulador não tem microfone (pico = 0 em 4 gravações, `tccd` sem evento) — não é permissão nem configuração. O ADR-0054 escolhe o substituto que o 0048 deixou em aberto, e o `apiBaseUrl` passa a derivar do host do bundler |

*Esta skill cresce pelos postmortems (`docs/learnings/`), não por antecipação.*
| 2026-09-11 | Regra nova: **soltar objeto nativo não o desliga — `pause()` antes de `remove()`** | CARD-042. Lido no Swift do `expo-audio@57.0.4` instalado e **medido** no Simulador: `remove()` sozinho cai em `registryQueue.async` e não chama `teardownPlayer()`, logo não pausa nada — o `currentTime` avançou **2008–2300 ms** (o resto inteiro de um arquivo de 2,3 s); com `pause()` antes, **1–2 ms**. Ver LEARNING-0006 |
| 2026-09-11 | Gate novo: **`vitest run` entra como terceiro anel do cliente** | CARD-042 / ADR-0061. O gatilho estava escrito no ADR-0043 item 6 e disparou: a ordem `pause()` → `remove()` é invisível para lint, tipo e tela. A dívida "cliente sem gate de teste", registrada aqui em 2026-08-24, **fecha em parte** — a fronteira `apps/` ↔ `packages/` segue sem gate |

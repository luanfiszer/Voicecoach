...
| 2026-08-23 | `application/use_cases/` entrou no mapa "onde mora o quê" | CARD-009 criou o primeiro handler do projeto. Até aqui a linha dizia só `application/ (handler CQS)`, sobre um diretório que não existia |
| 2026-08-23 | Quatro linhas novas sobre o worker: `ctx`, string compartilhada, canal de eventos, concorrência | ADRs 0035–0038. O worker deixou de ser um `__init__.py` vazio depois de quatro cards |
| 2026-08-23 | Proibição "`adapters` importar `worker`" acrescentada, com o caso real | Não era hipótese: o `check_worker` do health importou `READY_KEY` de `worker/readiness.py` e o `lint-imports` reprovou. A skill dizia só "api ↔ worker", e a violação foi por outro par |
| 2026-08-23 | `arq` e `hiredis` nas listas `forbidden` de `domain` e `application` | ADR-0038, no mesmo commit da dependência. Par completo demonstrado (fora da lista → 1 broken; dentro → 2 broken) |
| 2026-08-23 | `Result` (ADR-0017) segue TBD, agora com o gatilho **conferido e negado** por escrito | CARD-009 inventariou os desfechos de falha do pipeline: todos são infraestrutura. Registrar a conferência é o que separa decisão de esquecimento |
...
| 2026-08-23 | **O `Result` deixou de ser TBD**: forma decidida, com a regra de fronteira entre exceção e `Result` numa tabela | ADR-0039, CARD-010. A skill dizia "continua TBD" desde o CARD-005 e passaria a mentir na primeira linha do handler novo. O achado que mudou o desenho: `Idempotency-Key` repetida — um dos três exemplos do gatilho do ADR-0017 — **não é falha**, é um segundo sucesso |
| 2026-08-23 | Formato de erro HTTP (Problem Details) virou regra com lugar (`api/errors.py`) e tabela de códigos | ADR-0040, CARD-010. O ADR-0008 item 5 e o ADR-0017 item 2 **prometiam** essa borda desde 2026-08-17 e ela nunca tinha sido escrita — não havia `exception_handler` nenhum no app |
| 2026-08-23 | Entraram `lifespan`, composição por request, `id` do SSE e idempotência por coluna | ADRs 0040–0042, CARD-010. A API deixou de ser só o health check, e nenhuma dessas quatro perguntas ("onde mora o pool?", "onde monto o handler?") tinha resposta na skill |

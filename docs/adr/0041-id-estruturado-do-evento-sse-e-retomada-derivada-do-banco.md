...
5. **`feedback` não volta na retomada, e isso está escrito.** Um cliente que
   reconecte depois de o feedback ter passado não o recebe naquele turn; ele o vê
   no histórico, depois. Inventar um evento vazio seria pior que não mandar nada.
   **Gatilho para reabrir:** o CARD-013, que persiste as correções — a partir
   dele o `feedback` passa a ser reconstituível e entra no histórico como os
   outros.
...
- **O `feedback` continua sendo o buraco**, agora com o buraco documentado e
  testado (há um teste que afirma que ele **não** aparece no histórico). É
  dívida do ADR-0035, não desta decisão — mas é aqui que ela fica visível ao
  aluno.

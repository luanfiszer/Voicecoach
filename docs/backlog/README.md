...
| [042](CARD-042-regravar-cala-o-professor.md) | **Regravar cala o professor** — o bug do playback que sobrevive ao `limpar()` | 2 | mobile | P | 037, ADR-0047 | **parcial (2026-09-11)** — causa **confirmada na fonte**: `remove()` do `expo-audio` não chama `teardownPlayer()`, logo **não pausa nada**. Corrigido (`pause()` antes de `remove()`, geração na fila, guarda no `recuperarAudio`), com [ADR-0061](../adr/0061-o-primeiro-teste-do-cliente-vitest-sobre-logica-extraida.md) e [LEARNING-0006](../learnings/0006-remove-nao-e-dispose-e-o-gate-que-faltava-era-o-teste.md). **Dívida: os 200 ms no aparelho** |

...
| [040](CARD-040-nao-entendi-como-desfecho-do-turn.md) | **"Não entendi, pode repetir?"** vira desfecho de turn — e custa zero | 2 | backend/mobile | M | 039, ADR-0057 | **concluído (2026-09-13)** — `Turn.reject()`, `TurnStage.NOT_UNDERSTOOD`, evento SSE `rejected`; `RejectionReason` ganhou um terceiro valor (`NOT_ENGLISH`) porque o STT com idioma fixo traduzia português com confidence alta; contrato pronto, UI (mobile) fica para outro card |
| [041](CARD-041-o-aluno-falou-portugues-o-professor-trata-isso.md) | **O aluno falou português, e o professor trata isso como professor** (cria o bloco de contexto do prompt) | 2 | backend/IA | M | 039, ADR-0059 | **bloqueado (2026-09-13)** — contradiz o ADR-0057 revisado no CARD-040 (português agora é `NOT_ENGLISH`, recusado antes do professor); precisa de decisão do desenvolvedor entre revogar este card ou reabrir o ADR-0057 |
| [044](CARD-044-a-voz-do-professor-escolhida-por-escuta.md) | **A voz do professor, escolhida por escuta comparada** | 2 | backend/IA | P | ADR-0032 | **bloqueado (2026-09-13)** — medido: trocar entre vozes `medium` é de graça; `high` custa **6,5x**. Falta só a escuta (preferência subjetiva de UX, fora do que o loop autônomo decide); instrumento já existe em `benchmarks/tts_audicao.py` |
| [043](CARD-043-cancelar-o-turn-no-servidor.md) | **Cancelar o turn no servidor** — o caminho que o V2 vai cobrar caro | 2 | backend/mobile | M | 042, ADR-0058 | **adiado (2026-09-13)** — recuo do próprio ADR-0058 (alternativa A): levantamento mostrou >12 arquivos em toda camada, um risco concreto de `aclose()` no gerador do professor, e falta de autenticação no backend inteiro (não só aqui) para "só o dono cancela". CARD-042 já cala o cliente; produto não regride, só continua pagando o teto conhecido |
| [045](CARD-045-nivel-declarado-no-onboarding.md) | **O nível declarado pelo aluno entra na conversa** — fecha o buraco visão × backlog | 3 | backend/mobile | M | 041, ADR-0059 | **bloqueado (2026-09-13)** — duas causas independentes: o mecanismo do CARD-041 nunca foi construído (bloqueado), e `/v1/students/me` pressupõe autenticação que não existe em nenhuma rota (CARD-049+) |
...
| [015](CARD-015-quotas-e-kill-switch.md) | Quotas + kill switch *(bloqueante comercial)* | 2 | backend | M | 010, 014 | **concluído (2026-09-13)** — ADR-0063: cota por student (minutos+turns), kill switch global por custo, rate limit por sessão+IP (Redis, sem migration). Decisão autônoma **pendente de revisão**: rate limit usa `session_id` no lugar de "conta" (sem auth ainda) |
...
| [016](CARD-016-ui-de-correcoes-no-app.md) | UI de correções + resumo de sessão no app | 3 | mobile | M | 012, 013 | **concluído (2026-09-13)** — migrado do objeto legado (só a 1ª correção) para `corrections[]` tipado, com badge de tipo/severidade por card; corrigida a bolha "SEM ERROS" que contrariava "sem correções ⇒ sem card"; resumo mínimo por sessão. Botão `traduzir` fica para quando o CARD-036 existir |
| [027](CARD-027-telas-de-excecao-do-app.md) | **Telas de exceção: offline, quota, pausado, timeout** | 3 | mobile | M | 015, 025, 026 | **concluído (2026-09-13)** — `OverlayDeExcecao` + `conteudoDaExcecao.ts` (ADR-0061); fila local offline **cortada de escopo** (exigiria `expo-file-system`, dependência nova sem ADR) — retry manual na mesma sessão fica; achado e corrigido gap pré-existente: `ErroDaApi` descartava o `type` do Problem Details (ADR-0040); cota e travado verificados ponta a ponta no Simulador (backend congelado com `kill -STOP` para simular o timeout de 30s); pausado coberto só por teste unitário |
| [029](CARD-029-historico-de-sessoes-no-app.md) | **Histórico de sessões no app** (+ `GET /v1/sessions` e abas) | 3 | backend/mobile | M | 013, 016, 017 | **concluído** |
| [030](CARD-030-consulta-de-sessoes-listagem-agregada.md) | Backend do histórico: `GET /v1/sessions` agregado + mídia expirada | 3 | backend | M | 013, 017 | **concluído** |
...
| [035](CARD-035-controles-do-player-sobre-a-fila-de-trechos.md) | Controles do player sobre a fila: `0.75×`, `repetir`, scrub | 3 | mobile | M | 028, 012 | **desbloqueado (2026-09-13)** — o aparelho físico com áudio real que faltava já existe (testado nesta data); corte sugerido no card continua valendo se o esforço estourar |
| [036](CARD-036-traducao-sob-demanda.md) | Tradução sob demanda: o endpoint do botão `traduzir` | 3 | backend | P | 013, 014, 026 | **concluído** |
| [049](CARD-049-cadastro-login-e-o-par-de-tokens.md) | **Cadastro, login e o par de tokens** — a auth que o ADR-0007 desenhou | 3 | backend | M | ADR-0007, **ADR de provedor de e-mail** | **concluído (2026-09-13)** — e-mail+senha, JWT+refresh rotativo com detecção de reuso, verificação de e-mail, recuperação de senha (adicionada nesta sessão). [ADR-0068](../adr/0068-provedor-de-email-transacional-resend-com-console-como-default.md) (Resend). Todas as rotas existentes migraram para o token real — **quebra o app até o CARD-050**. Login social virou CARD-060 |
| [050](CARD-050-a-sessao-autenticada-no-cliente.md) | **A sessão autenticada no cliente** — secure storage, refresh e expiração | 3 | mobile | M | 049 (concluído), ADR-0007 | **concluído (2026-09-13)** — `expo-secure-store`, refresh com promessa compartilhada (dedup testado: duas chamadas concorrentes com `401` disparam um único refresh), cinco telas de auth, `useTurno`/`useHistorico` migrados para `fetchAutenticado`. Nada rodou em aparelho físico ainda (dívida declarada) |
| [051](CARD-051-delete-de-conta-dentro-do-app.md) | **Delete de conta dentro do app** | V1 | backend/mobile | M | 049, 050, 017, [ADR-0069](../adr/0069-delete-de-conta-conteudo-apaga-usageevent-sobrevive-anonimo.md) | concluído — bloqueante de V1.0 (N4) fechado |
...
| [057](CARD-057-a-imagem-do-minio-saiu-do-docker-hub.md) | **A imagem do MinIO saiu do Docker Hub** — o CI do backend quebrou e o compose não sobe numa máquina nova | V1 | infra/backend | M | ADR-0062; relac. 055 | **concluído (2026-09-13)** — imagem trocada para `quay.io/minio/minio`, mesma tag, nos três lugares; teste de consistência compose↔teste novo; `pull` no CI antes do pytest; `test_url_assinada_expira` sem `sleep` fixo (8 execuções seguidas sem piscar) |
| [058](CARD-058-botao-traduzir-no-app.md) | **O botão `traduzir` ganha tela** — o adendo que o CARD-016 prometeu quando o CARD-036 existisse | 3 | mobile | P | 016, 036 | **concluído (2026-09-13)** — `traduzirTexto` no `api-client`, estado local por turn em `useTurno` (alvo sempre `reply`, sem re-tradução automática), botão na bolha da resposta em `ListaDoTurno.tsx` |
| [059](CARD-059-tela-de-configuracoes.md) | **Configurações: quarta aba**, para preferência de aparelho (não de conta) | 3 | mobile | P | 029; item de reprodução depende do 035 | **concluído com escopo reduzido (2026-09-13)** — seções 2 (limite de gravação, leitura) e 3 (sobre/versão) implementadas; seção 1 (preferências de reprodução) adiada até o CARD-035 existir |
...
| [060](CARD-060-login-social-google-e-apple.md) | **Login social: Google e Sign in with Apple, os dois juntos** | 3 | backend/mobile | M | 049, **ADR novo** | backlog — criado em 2026-09-13, a pedido do desenvolvedor durante o CARD-049. Google sem Apple viola a Guideline 4.8 da App Store, então não é "card menor", é a mesma decisão |
